...
our $HttpConfig = qq{
    ...
    server { listen 12354; location = /status { return 200; } }

    server { listen 12355; location = /status { return 502; } }

    server { listen 12356; location = /status { return 404; } }

    server { listen 12357; location = /status { content_by_lua ' ngx.sleep(3) ngx.status = 200 '; } }

    init_by_lua '
        local config = require "config_feedback"
...
--- config
    location = /t {
        ...
        content_by_lua '
            ...
            local ok, err = checkups.feedback_status("status", "127.0.0.1", 12354, true)
            if not ok then
                ngx.say(err)
            end

            local ok, err = checkups.feedback_status("status", "127.0.0.1", 12356, true)
            if not ok then
                ngx.say(err)
            end
